...
module Codchi.Platform.Windows (runCodchiLIO) where
...
runCodchiLIO = interpret $ \case
    ...
    ControllerStart -> runCodchiLIO $ rethrowAll mainLoop
    GetInstanceInfo name -> do
        wslInstances <- listWSLInstances
        let instances =
                [ CodchiInstance instanceName _wslStatus
                | WSLInstance{..} <- wslInstances
                , instanceName <-
                    maybeToList $ rightToMaybe . parse =<< Text.stripPrefix _INSTANCE_PREFIX _wslName
                ]
    ...
    RunCtrlNixCmd printLog cmd -> rethrowAll $ do
        let printWSL Win _ = pass
            ...
            rp
                | printLog = readProcessWith printWSL printWSL
            ...
            wrapIO $
                wsl'exeCmd $
                    ...
                    , "login"
                        ...
                        <> args
    UpdateShortcuts name swSharePath -> runCodchiLIO $ rethrowAll $ Codchi.Platform.Windows.updateShortcuts name swSharePath
...
updateShortcuts ::
    [IOE, Error ShellException, Error Panic, Logger, CodchiL] :>> es =>
    CodchiName ->
    FilePath ->
    Eff es ()
...
getFolderPath csidl = mkNTPath <$> sHGetFolderPath nullPtr csidl nullPtr 0
...
readProcessWith outLogger errLogger proc = do
    let proc' =
        ...
        splitWinWSL txt = case Text.splitOn _MAGIC_UTF8_SEQ txt of
            ...
            _ -> txt
    (exitCode, out, err) <-
        logTraceShowId "readProcess result"
    ...
    readProcess "wsl.exe --version" >>= \case
        ...
        Right _ ->
            readProcess "wsl.exe -l -v" >>= \case
                Right out -> do
                    instances <-
                        either (throwError . Panic . ParseException) return $
                            parse @[WSLInstance] out
                    case find ((== _CONTROLLER) . (._wslName)) instances of
                        Nothing -> return ControllerNotInstalled
                        Just status -> return $ ControllerInstalled status._wslStatus
    ...
    case res of
        ...
        Left ex
            | isPermissionError ex && maxRetries > 1 -> do
                let retries' = logTrace "retryOnPermissionErrors" (show @Text (ex, maxRetries)) (maxRetries - 1)
                threadDelay ((maxRetries - retries') * 200)
                retryOnPermissionErrors retries' io
            | otherwise -> throwIO $ logTraceShowId "retryOnPermissionErrors error" ex
