...
in
{
  imports = [
    ...
    (lib.mkRemovedOptionModule [ "codchi" "welcome" "text" ] "Please use `codchi.initScript` instead.")

    (lib.mkChangedOptionModule [ "codchi" "welcome" "extraText" ] [ "codchi" "initScript" ]
      (config: lib.pipe config.codchi.welcome.extraText [
        (lib.splitString "\n")
        (map (msgLine: "echo '${msgLine}'"))
        lib.concatLines
      ])
    )
