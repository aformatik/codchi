    ...
    let command = LXD_COMMAND
        .get_or_try_init(|| {
            if let Ok(_) = which("lxd") {
                tracing::trace!("Using LXD as container runtime.");
                Ok("lxc")
            } else if let Ok(_) = which("incus") {
                tracing::trace!("Using Incus as container runtime.");
                Ok("incus")
            } else {
        ...
        I: Iterator<Item = &'a LxdDevice>,
    {
        (|| {
            ...
            while !image::list()?
                ...
                .any(|img| img.aliases.iter().any(|alias| alias.name == name))
            {
                tracing::trace!("Waiting for LXD to import image '{name}'");
        ...
        .inspect_err(|_err| {
            tracing::error!("Removing leftovers of LXD container {name}...");
